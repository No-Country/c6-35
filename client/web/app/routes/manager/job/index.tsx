...
import { WorkOrderModel } from "~/server/workteam.server copy";

export const loader: LoaderFunction =async ({params,request}) => {
    const url = new URL(request.url);
    const created = url.searchParams.get("created");
    let employees: WorkOrderModel[];
    try {
        employees = [];
    } catch (error) {
...
export default function JobAdmin() {
    const {data} = useLoaderData() as {data:WorkOrderModel[]};
    const [workOrder, setWorkOrder] = useState(data);
    const [activedDeletedOption, setActivedDeletedOption] = useState(false);
    let navigate = useNavigate();
    const [columns, setColumns] = useState([
        {key:"code", name:"codigo"},
        {key:"observacion", name:"observacion", callBack:(obj:WorkOrderModel)=><><p>{obj.observacion}</p></>},
        {key:"direccion", name:"direccion", callBack:(obj:WorkOrderModel)=><><p>{`${obj.address.municipality.denominacion}, ${obj.address.location.denominacion}, ${obj.address.street} ${obj.address.number}`}</p></>},
        ].map(o=>({...o,on:true})));
    ...
    return (<>
        ...
        <div className="text-2xl">
                <h1>Administrar Ordenes de Trabajo</h1>
            </div>
